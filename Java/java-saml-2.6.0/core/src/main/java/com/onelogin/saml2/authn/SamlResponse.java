...
public class SamlResponse {
	...
	public SamlResponse(Saml2Settings settings, String currentUrl, String samlResponse) throws XPathExpressionException, ParserConfigurationException, SAXException, IOException, SettingsException, ValidationError {
		...
		LOGGER.debug("currentUrl: [{}] ClientDestinationURL: [{}]", currentUrl, settings.getClientDestinationURL());

		if(settings.getClientDestinationURL() != null) {
			LOGGER.debug("The current URL has been taken from configuration");
			this.currentUrl = settings.getClientDestinationURL();
		}
	...
	protected void validateDestination(final Element element) throws ValidationError {
		if (element.hasAttribute("Destination")) {
			...
			LOGGER.debug("destinationUrl: [{}] currentUrl: [{}]", destinationUrl, currentUrl);
