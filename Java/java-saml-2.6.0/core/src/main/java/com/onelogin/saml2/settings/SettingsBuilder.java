...
public class SettingsBuilder {
	...
	public final static String CLIENT_DESTINATION_URL = "onelogin.saml2.customer.custom.destination.url";

	private String loadClientDestinationURL() {
		return loadStringProperty(CLIENT_DESTINATION_URL);
	}
	...
	public SettingsBuilder fromFile(String propFileName, KeyStoreSettings keyStoreSetting) throws Error, IOException {
		...
		try (InputStream inputStream = classLoader.getResourceAsStream(propFileName)) {
			if (inputStream != null) {
				return this.fromFile(inputStream, keyStoreSetting);
			} else {
	...
	public SettingsBuilder fromFile(File propFile, KeyStoreSettings keyStoreSetting) throws Error, IOException {
		if(!propFile.exists() | !propFile.isFile() | propFile.canRead()) {
			LOGGER.error("Unable to load the specified settings file! File: [{}] isFile? {} exists? {} canRead? {}",
				propFile.getAbsolutePath(), propFile.exists(), !propFile.isFile(), propFile.canRead());
			throw new Error("Unable to load the specified settings file!", Error.SETTINGS_FILE_NOT_FOUND);
		}
		return this.fromFile(new FileInputStream(propFile), keyStoreSetting);
	}

	/**
	 * Load settings from the file
	 *
	 * @param propFile OneLogin_Saml2_Settings
	 ...
	 * @throws Error
	 */
	public SettingsBuilder fromFile(InputStream propFile, KeyStoreSettings keyStoreSetting) throws Error, IOException {
	...
	private void loadCustomSettings() {
		String clientDestinationURL = loadClientDestinationURL();
		LOGGER.debug("clientDestinationURL: [{}]", clientDestinationURL);
		if (StringUtils.isNotEmpty(clientDestinationURL)) {
			saml2Setting.setClientDestinationURL(clientDestinationURL);
		}
	}
