      ...
      <xsl:for-each select="QueryInfo/RowInfo">      
        <fo:page-sequence master-reference="hojaCarta">
          ...
          <fo:flow flow-name="xsl-region-body">
            ...
            <fo:block margin-top="5mm" font-size="7pt"></fo:block>
            <fo:block margin-top="5mm" font-size="7pt">
              Hacemos referencia al Fideicomiso
              <fo:inline font-weight="bold"><xsl:value-of select="ravNumContrato"/></fo:inline>
              <fo:inline font-weight="bold"><xsl:value-of select="ravNomContrato"/></fo:inline> y en particular a nuestro aviso de cobro, primer y segundo recordatorio de fechas 
              <fo:inline font-weight="bold"><xsl:value-of select="ravFechaAviso"/></fo:inline>, 
              <fo:inline font-weight="bold"><xsl:value-of select="ravFechaPri"/></fo:inline> y 
              <fo:inline font-weight="bold"><xsl:value-of select="ravFechaSeg"/></fo:inline> mediante los cuales les solicitamos el pago de los honorarios fiduciarios vencidos por un importe de $
              <fo:inline font-weight="bold"><xsl:value-of select="ravImporte90"/></fo:inline> IVA incluido. 
            </fo:block>
            <fo:block margin-top="5mm" font-size="7pt">En virtud de que a la fecha no nos han sido cubiertos los citados honorarios, el adeudo presenta una antigüedad de 90 días y que adicionalmente vencen los correspondientes al periodo 
              <fo:inline font-weight="bold"><xsl:value-of select="ravPeriodo"/></fo:inline> por un importe de $
              <fo:inline font-weight="bold"><xsl:value-of select="ravImportePer"/></fo:inline> IVA incluido, por lo que les solicitamos pago inmediato del mismo, ya que de lo contrario tendrá que ser turnado a nuestra área jurídica para su cobro por la vía judicial.
            </fo:block>
            <fo:block margin-top="5mm" font-size="7pt">Como es de su conocimiento, el pago lo podrá realizar mediante deposito en la cuenta cuyos datos a continuación se citan:</fo:block>  
              ...
              <fo:table>
                ...
                <fo:table-body>
                  ...
                  <fo:table-row>
                    ...
                    <fo:table-cell><fo:block text-align="left">$<xsl:value-of select="ravImporteTot"/></fo:block></fo:table-cell>
                  </fo:table-row>                
                </fo:table-body>              
              </fo:table>
            </fo:block>
