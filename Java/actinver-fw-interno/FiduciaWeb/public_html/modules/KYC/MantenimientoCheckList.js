  ...
  for(i=longitud;i<60;i++)//se eliminan los elementos que no se utilizan
    ...
    var resultados=JSON.parse(result)[i];
      ...
      if(resultados.ftcpLista==1){
        ...
        // boleanas direccion
        var bDelegacionMunicipio = resultados.ftcpEtiqueta.indexOf('DELEGACION O MUNICIPIO')!=-1;
        var bEntidadFederativa = resultados.ftcpEtiqueta.indexOf('ENTIDAD FEDERATIVA')!=-1;
        var bPais = resultados.ftcpEtiqueta.indexOf('PAIS')!=-1;
        
        if(bDelegacionMunicipio||bEntidadFederativa)//si es parte del domicilio
        {
                    parametro2 = bEntidadFederativa?valorcampo:parametro2;//estado
                    
                    var parametrosKycDomicilio ="{\"numConcept\":" + resultados.ftcpIdConceptos + ",\"menor\":" + ((bEntidadFederativa)?-100+parametro1*100:parametro2*10000) + ",\"mayor\":" + ((bEntidadFederativa)?parametro1*100:10000+parametro2*10000) + "}";
        ...
        {
                    parametro1 = bPais?valorcampo:parametro1;// pais
...
function pasaValor(ide)
{
  ...
  if(ide.indexOf('cmbSubconceptoAlt')==-1){
      ...
      if(GI('cmbSubconceptoAlt'+ide.replace('cmbSubconcepto','')).style.visibility=='hidden')
      {
         ...
         // boleanas direccion
        var bEntidadFederativa = etiqueta.indexOf('ENTIDAD FEDERATIVA')!=-1;
        var bPais = etiqueta.indexOf('PAIS')!=-1;
         
          if(bEntidadFederativa||bPais)
          {
              ...
              var menor = ((!bEntidadFederativa)?eval(-100+parametro1*100):eval(parametro1*10000));
              var mayor = ((!bEntidadFederativa)?eval(parametro1*100):eval(10000+parametro1*10000));
              
              var parametrosKycDomicilio ="{\"numConcept\":"+numConcept+"0,\"menor\":" + menor + ",\"mayor\":" + mayor + "}";
              //alert("Admon: "+parametrosKycDomicilio)
