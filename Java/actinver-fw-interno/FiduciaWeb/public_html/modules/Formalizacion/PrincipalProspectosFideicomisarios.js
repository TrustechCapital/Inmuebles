...
function despliegaPantallaMantenimientoProspectosFideicomisarios(obj, result) {
    ...
    fvMantenimientoFideicomisarios.setup( {
        formName : "frmDatosMantenimientoFideicomisarios", tipoAlert : 1, alertFunction : BaloonAlert, sendObjToAlert : true
    });
...
function respuestavalidaEtapa(obj, result){
  ...
  if(isDefinedAndNotNull(resultado) && resultado[0].existe> 0) {
    ...
    deshabilitaObjetos(GI("frmDatosMantenimientoFideicomisarios"));                   //Deshabilita objetos (excepto botones)
  } else {
...
function catLoaded() {
    ...
    else if (operacion == CONSULTAR || operacion == CONSULTARPR)//Si se trata de una consulta, deshabilitar
    {
        SA(GI("cmdCancelar"), "value", "Regresar");//Colocar la leyenda Regresar en vez de Cancelar al boton
        deshabilitaObjetos(GI("frmDatosMantenimientoFideicomisarios"));
        // muestraObjs("cmdKYC");
...
function ejecutaFuncionPorcentaje(tp, result) {
    ...
    if (res == 0) {
        ...
        else if (operacion == MODIFICAR && fvMantenimientoFideicomisarios.checkForm())//Se trata de una modificacion
        {
            showWaitLayer();
...
function eliminarRegistro() {
    if (pkInfo == null)
        ...
        var objelimina = JSON.parse("{\"id\":\"borraFunPrev\",\"proyecto\":" + pkInfo.afbAnteproyecto + ",\"tipo\":\"" + pkInfo.afbCvePersona + "\"" + ",\"persona\":" + pkInfo.afbNumFidben + "}");
...
function validaNombre() {
    if (!/^([a-zA-Z\s])*$/.test(GI("afbNomFidben").value)) {
        alert("Verificar nombre");
        GI("afbNomFidben").value = "";
    }
    else {
        return (true);
    }
...
function validaParticipacion() {
    var validNum = /^[0-9]*$/;
    if (!validNum.test(GI("afbCurp").value)) {
...
/*function ValidateEmail() {
    if (/^\w+([\.-]?\w+)@\w+([\.-]?\w+)(\.\w{2,3})+$/.test(vgContenedorDatos.Email = GI("afbMailFidben").value)) {
        GI("afbMailFidben").value = "";
        return (true);
