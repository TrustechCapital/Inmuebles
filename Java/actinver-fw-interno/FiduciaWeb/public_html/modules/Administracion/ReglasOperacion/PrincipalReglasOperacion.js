...
function AltaOModificaInfo() {
  ...
  if(operacion==ALTA )//Se trata de una alta && fvMantenimientoFideicomisos.checkForm()
  {
...
function mostrarDatosInformativos(parametroPantalla) {
  ...
  else if(parametroPantalla==2)
  {
    ...
    showWaitLayer();
      validacionAlta2.numContrato = GI("froNumFideicom").value;
      var url = ctxRoot + "/getRef.do?json=" + JSON.stringify(validacionAlta2);
      makeAjaxRequest(url, "HTML", funcVerExistRegistro2, GI("froNumFideicom"));
  }
  else
  {
    showWaitLayer();
...
function funcVerExistRegistro2(obj,result) {
  var objResult = JSON.parse(result);
  if(objResult[0].ctoCveStContrat == 0)
  {
    consultaDatosInformativos.NumFideicomiso = GI("froNumFideicom").value;
...
function insertaDatosInformativos(obj, result) {
  var objResult = JSON.parse(result);
  //GI("nomFideicomiso").value=objResult[0].ctoNomContrato;
  ...
  var contenedorDatos=JSON.parse("{\"id\":\"conNomFid\",\"numFideicomiso\":0}");
   contenedorDatos.numFideicomiso=obj.value;
    var url = ctxRoot + "/getRef.do?json=" + JSON.stringify(contenedorDatos);
    makeAjaxRequest(url, "HTML", setNombre, obj);
  //hideWaitLayer();
}
function setNombre(obj, result) {
  var objResult = JSON.parse(result);
  //alert(result);
  //alert("Nombre:"+objResult[0].nombre);
  GI("nomFideicomiso").visible=true;
  GI("nomFideicomiso").value=objResult[0].nombre;
  hideWaitLayer();
}
...
function setNumFideico(){
    numFideicomiso=GI("froNumFideicom").value;
}