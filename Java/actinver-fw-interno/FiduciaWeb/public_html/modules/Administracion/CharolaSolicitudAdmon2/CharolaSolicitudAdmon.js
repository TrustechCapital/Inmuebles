  ...
  var asignaPerfil = JSON.parse("{\"numEtapa\":\""+eval(10)+"\"}");
  ...
  //***FUNCIONALIDAD BOTONES*********************************************************************************************************************
    function funcionDelBoton(opc){
       if(isDefinedAndNotNull(pkInfoCharSol)){
          ...
          paramsQryDocumentsCbo = JSON.parse("{\"numOper\":\"" +pkInfoCharSol.insNumOper + "\",\"numEtapa\":\""+eval(10)+"\"}");
          showWaitLayer();
...
function recogerDatosParaPantallaMantenimiento(opc){
  ...
  function cargaTipoPantalla(){
       if(opcion==vgFunPtoRev){
          var urlCliente = ctxRoot + "/modules/Administracion/CharolaSolicitudAdmon2/PuntosRevisionSolicitudesAdmon.do";
          makeAjaxRequest(urlCliente, "HTML", despliegaPantalla, null);
       }
       else{
          var urlCliente = ctxRoot + "/modules/Administracion/CharolaSolicitudAdmon2/DocumentosSolicitudesAdmon.do";
          makeAjaxRequest(urlCliente, "HTML", despliegaPantalla, null);
       }
    }
  ...
  function regresarAlaCharola(){
    onButtonClick("Administracion/CharolaSolicitudAdmon2/CharolaSolicitudAdmon","");
    hideWaitLayer();
  }
  ...
  function despliegaTabla(tabla,obj){
    vgObjqryllenaTablaAdmon=JSON.parse("{\"id\":\"qryllenaTablaAdmon\"}");
          ...
          vgObjqryllenaTablaAdmon.numEtapa=eval(10);
  ...
  function resultadoConsulta(objTabla, result) {
    ...
    if(isDefinedAndNotNull(resultado) && resultado.length>0){
      ...
      alert("No existen Registros para estos criterios de busqueda");
      regresarAlaCharola();
      hideWaitLayer();
     } 
  }
  ...
  function autorizaRechaza(opc){
	 ...
	 if(!GI("SubeSolicitudChk").checked&&opc==1) 
	 alert("Debe subir la imagen para Autorizar la Solicitud!")
	else if(isDefinedAndNotNull(pkInfoCharSol)){ 
        vgContenedorDatos=null;
        vgContenedorDatos=JSON.parse("{\"id\":\"funcionAceptarRechazarCharolaSol\"}");
        vgContenedorDatos.opcion=eval(opc);
        vgContenedorDatos.numEtapa=eval(10);
     ...
     case  0: 
      if(vgopcAutRech!=1){
       ...
       {*/
        ...
        vgContenedorDatos.numOper=pkInfoCharSol.insNumOper;
        vgContenedorDatos.usuario=eval(GI("txtuserId").value)
		if(GI("SubeSolicitudChk").checked)
			vgContenedorDatos.subioImagen=eval(1)
		else
      ...
      break;
     case  1: alert("No se puede autorizar asta instruccion debido a que no todos sus puntos de revision han sido correctos"); consultar(GI("cmdAceptar"), GI("frmCharolaSolicitudes"), false); break;
     case  2: alert("No se puede autorizar asta instruccion debido a que no todos sus documentos tienen sus puntos de revision correctos"); break;
...
function regresarAlaCharola2(){
    ...
    var urlCliente = "modules/Administracion/CharolaSolicitudAdmon2/CharolaSolicitudAdmon.do";
  ...
  function ubicaFolioSolicitado(obj, result){
    ...
    GI("paramnumEtapa").value=eval(10);    
    ...
    SA(botonTemp,"ref","qryCharolInstrucAdmon2");
