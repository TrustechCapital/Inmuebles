...
var asignaPerfil = JSON.parse("{\"numEtapa\":\""+eval(4)+"\"}");
...
function funcionDelBoton(opc){
   if(isDefinedAndNotNull(pkInfoCharSol)){
	  ...
	  paramsQryDocumentsCbo = JSON.parse("{\"numOper\":\"" +pkInfoCharSol.insNumOper + "\",\"numEtapa\":\""+eval(4)+"\"}");
	  showWaitLayer();
...
function cargaTipoPantalla(){
   if(opcion==vgFunPtoRev){
	  var urlCliente = ctxRoot + "/modules/Administracion/CharolaSolicitudAdmon3/PuntosRevisionSolicitudesAdmon.do";
	  makeAjaxRequest(urlCliente, "HTML", despliegaPantalla, null);
   }
   else{
	  var urlCliente = ctxRoot + "/modules/Administracion/CharolaSolicitudAdmon3/DocumentosSolicitudesAdmon.do";
	  makeAjaxRequest(urlCliente, "HTML", despliegaPantalla, null);
   }
}
...
onButtonClick("Administracion/CharolaSolicitudAdmon3/CharolaSolicitudAdmon","");
hideWaitLayer();
}
...
vgObjqryllenaTablaAdmon=JSON.parse("{\"id\":\"qryllenaTablaAdmon\"}");
	  ...
	  vgObjqryllenaTablaAdmon.numEtapa=eval(4);
...
function autorizaRechaza(opc){
 ...
 if(isDefinedAndNotNull(pkInfoCharSol)){ 
	...
	vgContenedorDatos.numEtapa=eval(4);
...
switch(r){
 ...
 case  0: 
  ...
  break;
 case  1: alert("No se puede autorizar esta instruccion debido a que no todos sus puntos de revision han sido correctos"); consultar(GI("cmdAceptar"), GI("frmCharolaSolicitudes"), false); break;
 case  2: alert("No se puede autorizar esta instruccion debido a que no todos sus documentos tienen sus puntos de revision correctos"); break;
...
var urlCliente = "modules/Administracion/CharolaSolicitudAdmon3/CharolaSolicitudAdmon.do";
...
function viewDoc(objBtn) {
	...
	switch(parseInt(objSel.value)) {
		case 100: //Solicitud de Revocacion Firmada
			...
			break;
		case 101: //Identificacion Oficial vigente
