...
function frameSubmit(forma) {
    ...
    if(GI("paramFecha").value=="" ||
        ...
        if(GI("tipoXLS").checked) {
            ...
            $.post(ctxRoot + "/tr.do", {sid: trData.id}, function(data) {
                uploadMonitor(data);
            });
...
var urlCtl = ctxRoot + "/tr.do?sid=" + JSON.stringify({id:"cuentaAP"});
var stopCtl = false;
var procID = 0;
var veces = 0;
var ultimaCantidad = -1;
var trResp = {};
function uploadMonitor(srvData) {
    if(!stopCtl) { 
        setTimeout(function() {
            try {
                $.post(ctxRoot + "/tr.do", {sid: trData.id}, function(data) { 
                    console.log('TR: ' + data);
                    trResp = JSON.parse(data);
                    console.log('trResp.data: ' + trResp.data);
                    if(trResp.data != "" && trResp.data != "{}") {
                        trResp = JSON.parse(trResp.data);
                        stopCtl = true;
                        console.log('Monitor should be stopped!');
                        recuperaTAS();
                    } else {
                        uploadMonitor(trResp);    
                    }
                });
            } catch(e) {
...
function consultaControl2() {
    ...
    date = GI("paramFecha").value;
    var fechaEnPartes = date.split('/');
    var objCargaMasivaParam;
    /*if(eval(GI("slcReportes").value)==1){
        objCargaMasivaParam= JSON.parse("{\"id\":\"qryTransfApli\"}");
    }else{//tipo = reportos
        objCargaMasivaParam= JSON.parse("{\"id\":\"qryTransfApliReportos\"}");
    } */
    objCargaMasivaParam= JSON.parse("{\"id\":\"qryTransfApli\"}");
...
function setTR() {
    if(isDefinedAndNotNull(trData)) {
        try {
            document.getElementsByName('frameUpload')[0].document.frameUpload.document.getElementById('trData').value = trData.id;
        } catch(e) {
            console.log('Error setting the TR id. Error: ' + e.message);
        }
    }
}
...
function setTRID() {
    showWaitLayer();
    var url = ctxRoot + "/tr.do";
    makeAjaxRequest(url, "HTML", function(obj, result) {
        hideWaitLayer();
        trData = JSON.parse(result);
    }, null);
}
