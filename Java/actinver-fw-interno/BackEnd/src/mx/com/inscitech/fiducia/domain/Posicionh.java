...
@PrimaryKey(constraintName = "POSICIONH_PK", columns = {"POS_ANO_MOVTO", "POS_MES_MOVTO", "POS_NUM_CONTRATO", "POS_SUB_CONTRATO", "POS_NUM_ENTID_FIN", "POS_CONTRATO_INTER", "POS_CVE_TIPO_MERCA", "POS_NUM_INSTRUME", "POS_NUM_SEC_EMIS"}, sequences = { "MANUAL" })
public class Posicionh extends DomainObject {
  ...
  BigDecimal posAnoMovto = null;
  BigDecimal posMesMovto = null;
  ...
  @FieldInfo(nullable = false, dataType = "NUMBER", precision = 4, scale = 0, javaClass = BigDecimal.class )
  public void setPosAnoMovto(BigDecimal posAnoMovto) {
    this.posAnoMovto = posAnoMovto;
  }

  public BigDecimal getPosAnoMovto() {
    return this.posAnoMovto;
  }

  @FieldInfo(nullable = false, dataType = "NUMBER", precision = 2, scale = 0, javaClass = BigDecimal.class )
  public void setPosMesMovto(BigDecimal posMesMovto) {
    this.posMesMovto = posMesMovto;
  }

  public BigDecimal getPosMesMovto() {
    return this.posMesMovto;
  }

  public DMLObject getSelectByPK() {
    if(!retrieveSQL) return null;
    DMLObject result = new DMLObject();
    String sql = "SELECT * FROM POSICIONH ";
    ...
    if(this.getPosAnoMovto() != null && this.getPosAnoMovto().longValue() == -999) {
      conditions += " AND POS_ANO_MOVTO IS NULL";
    } else if(this.getPosAnoMovto() != null) {
      conditions += " AND POS_ANO_MOVTO = ?";
      values.add(this.getPosAnoMovto());
    }

    if(this.getPosMesMovto() != null && this.getPosMesMovto().longValue() == -999) {
      conditions += " AND POS_MES_MOVTO IS NULL";
    } else if(this.getPosMesMovto() != null) {
      conditions += " AND POS_MES_MOVTO = ?";
      values.add(this.getPosMesMovto());
    }
  ...
  public DMLObject getUpdate() {
    if(!retrieveSQL) return null;
    DMLObject result = new DMLObject();
    String sql = "UPDATE POSICIONH SET ";
    ...
    values.add(this.getPosMinusPlus());
    conditions += " AND POS_ANO_MOVTO = ?";
    pkValues.add(this.getPosAnoMovto());
    conditions += " AND POS_MES_MOVTO = ?";
    pkValues.add(this.getPosMesMovto());
  ...
  public DMLObject getInsert() {
    if(!retrieveSQL) return null;
    DMLObject result = new DMLObject();
    String sql = "INSERT INTO POSICIONH ( ";
    ...
    fields += ", POS_ANO_MOVTO";
    fieldValues += ", ?";
    values.add(this.getPosAnoMovto());

    fields += ", POS_MES_MOVTO";
    fieldValues += ", ?";
    values.add(this.getPosMesMovto());
  ...
  public DMLObject getDelete() {
    if(!retrieveSQL) return null;
    DMLObject result = new DMLObject();
    String sql = "DELETE FROM POSICIONH WHERE ";
  ...
  public boolean doCompare(Object compareWith) {
    Posicionh instance = (Posicionh)compareWith;
    ...
    if(equalObjects && !this.getPosAnoMovto().equals(instance.getPosAnoMovto())) equalObjects = false;
    if(equalObjects && !this.getPosMesMovto().equals(instance.getPosMesMovto())) equalObjects = false;
    return equalObjects;
  }

  public Object selectAsObject() {
    Posicionh result = new Posicionh();
    ...
    result.setPosAnoMovto((BigDecimal)objectData.getData("POS_ANO_MOVTO"));
    result.setPosMesMovto((BigDecimal)objectData.getData("POS_MES_MOVTO"));
