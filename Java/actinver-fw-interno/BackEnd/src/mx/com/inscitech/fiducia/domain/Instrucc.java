...
@PrimaryKey(constraintName = "INSTRUCC_PK", columns = { "INS_NUM_FOLIO_INST", "INS_NUM_CONTRATO" },
            sequences = { "MANUAL" })
public class Instrucc extends DomainObject {
    ...
    @FieldInfo(nullable = true, dataType = "VARCHAR2", javaClass = String.class)
    public void setInsNumOper(String insNumOper) {
        this.insNumOper = insNumOper;
    }

    public String getInsNumOper() {
        return this.insNumOper;
    }
    ...
    public DMLObject getSelectByPK() {
        ...
        String sql = "SELECT * FROM INSTRUCC ";
    ...
    public DMLObject getSelect() {
        ...
        if (this.getInsNumOper() != null && "null".equals(this.getInsNumOper())) {
            conditions += " AND INS_NUM_OPER IS NULL";
        } else if (this.getInsNumOper() != null) {
            conditions += " AND INS_NUM_OPER = ?";
            values.add(this.getInsNumOper());
        }
    ...
    public DMLObject getUpdate() {
        if (!retrieveSQL)
            return null;
        DMLObject result = new DMLObject();
        String sql = "UPDATE INSTRUCC SET ";
    ...
    public DMLObject getInsert() {
        if (!retrieveSQL)
            return null;
        DMLObject result = new DMLObject();
        String sql = "INSERT INTO INSTRUCC ( ";
    ...
    public DMLObject getDelete() {
        if (!retrieveSQL)
            return null;
        DMLObject result = new DMLObject();
        String sql = "DELETE FROM INSTRUCC WHERE ";
    ...
    public boolean doCompare(Object compareWith) {
        Instrucc instance = (Instrucc) compareWith;
        ...
        if (equalObjects && !this.getInsNumFolioInst().equals(instance.getInsNumFolioInst()))
            equalObjects = false;
        if (equalObjects && !this.getInsSubContrato().equals(instance.getInsSubContrato()))
            equalObjects = false;
        if (equalObjects && !this.getInsTxtComentario().equals(instance.getInsTxtComentario()))
            equalObjects = false;
        if (equalObjects && !this.getInsCveTipoInstr().equals(instance.getInsCveTipoInstr()))
            equalObjects = false;
        if (equalObjects && !this.getInsNumMiembro().equals(instance.getInsNumMiembro()))
            equalObjects = false;
        if (equalObjects && !this.getInsNomMiembro().equals(instance.getInsNomMiembro()))
            equalObjects = false;
        if (equalObjects && !this.getInsAnoAltaReg().equals(instance.getInsAnoAltaReg()))
            equalObjects = false;
        if (equalObjects && !this.getInsMesAltaReg().equals(instance.getInsMesAltaReg()))
            equalObjects = false;
        if (equalObjects && !this.getInsDiaAltaReg().equals(instance.getInsDiaAltaReg()))
            equalObjects = false;
        if (equalObjects && !this.getInsAnoUltMod().equals(instance.getInsAnoUltMod()))
            equalObjects = false;
        if (equalObjects && !this.getInsMesUltMod().equals(instance.getInsMesUltMod()))
            equalObjects = false;
        if (equalObjects && !this.getInsDiaUltMod().equals(instance.getInsDiaUltMod()))
            equalObjects = false;
        if (equalObjects && !this.getInsCveStInstruc().equals(instance.getInsCveStInstruc()))
            equalObjects = false;
        if (equalObjects && !this.getInsCveStCont().equals(instance.getInsCveStCont()))
            equalObjects = false;
        if (equalObjects && !this.getInsFechaContable().equals(instance.getInsFechaContable()))
            equalObjects = false;
        ...
        if (equalObjects && !this.getAcuId().equals(instance.getAcuId()))
            equalObjects = false;
        if (equalObjects && !this.getInsNumOper().equals(instance.getInsNumOper()))
            equalObjects = false;
        return equalObjects;
    }

    public Object selectAsObject() {
        Instrucc result = new Instrucc();
        ...
        result.setInsNumOper((String) objectData.getData("INS_NUM_OPER"));
