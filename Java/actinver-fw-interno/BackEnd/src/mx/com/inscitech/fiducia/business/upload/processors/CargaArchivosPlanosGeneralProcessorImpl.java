  ...
  public void run() { 
    ...
    try {
      ...
      while(tasFileReader.ready()) {
        ...
        if(!archivosPlanos.doInsert()) throw new Exception("Error al registrar el archivo en Base de Datos.");
      }

    } catch(Exception e) {
        logger.log(this, Thread.currentThread(), LoggingService.LEVEL.ERROR, "Error en la carga de archivos planos.", e);
       // System.out.println("Error en la carga de archivos planos General.");
    } finally { 
    }
  }
