...
public class TasProcessorImpl extends UploadProcessor {
    ...
    static {
        ...
        bodyInfo.add(new FieldInfo("Slash Fecha", "Slash de la fecha de liquidacion", new BigDecimal(1), null));
        ...
        try {
            ...
            while (tasFileReader.ready()) {
                ...
                if (isHeader) {
                    ...
                    if (tipoReg.equals("3")) {
                        ...
                        for (i = 0; i < bodyInfo.size(); i++) {
                            ...
                            if (field.getMapToField() == null || field.getMapToField().equals("")) {
                                ...
                                /*try {
                                  System.out.println("methodName: " + methodName + " Class: <" + field.getTypeLengthInfo().getClass() + "> BigDecimal: <" + BigDecimal.class + "> Equals? " + (field.getTypeLengthInfo().getClass() == BigDecimal.class));
                                  if(field.getTypeLengthInfo().getClass() == BigDecimal.class) {
                                    System.out.println("Value Pre: '" + fieldValue + "'");
                                    fieldValue = "" + new BigDecimal(fieldValue.trim()).doubleValue();
                                    System.out.println("Value Post: '" + fieldValue + "'");
                                  }
                                } catch (Exception e) { }*/
                    ...
                    else if (tipoReg.equals("6")) { // codigo ARCHIVOSPLANOS---PRECIO PROMEDIO

                        archivosPlanos = new ArchivosPlanos();

                        archivosPlanos.setArpSecuencial(new BigDecimal(secuencial++));
                        archivosPlanos.setArpFecha((String) this.getParameters().get("fechaTas"));
                        archivosPlanos.setArpNomArchivo("PRECIO PROMEDIO");
                        archivosPlanos.setArpDescripcion(linea);
